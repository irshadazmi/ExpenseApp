...
from app.schemas.budget_schema import BudgetResponseSchema, BudgetCreateSchema, BudgetUpdateSchema
...
async def create_budget(budget: BudgetCreateSchema, session: AsyncSession = Depends(get_db)):
    budget_repo = BudgetRepository(session)
    budget_service = BudgetService(budget_repo)
    return await budget_service.create_budget(budget)

@budget_router.put("/{budget_id}", response_model=BudgetResponseSchema)
async def update_budget(budget_id: int, budget: BudgetUpdateSchema, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        raise InternalServerErrorException("Failed to update budget "+str(e))
