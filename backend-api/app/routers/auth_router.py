...
from app.schemas.auth_schema import AuthRegisterSchema, AuthLoginSchema, AuthResponseSchema, AuthResultSchema, AuthUpdateSchema
...
async def get_me(token: str = Depends(oauth2_scheme), session: AsyncSession = Depends(get_db)):
...
@auth_router.post("/register", response_model=AuthResultSchema, status_code=status.HTTP_201_CREATED)
async def register(user: AuthRegisterSchema, session: AsyncSession = Depends(get_db)):
...
@auth_router.post("/login", response_model=AuthResultSchema, status_code=status.HTTP_200_OK)
async def login(credentials: AuthLoginSchema, session: AsyncSession = Depends(get_db)):
    try:
        auth_service = AuthService(AuthRepository(session))
...
async def update_user(user_id: int, user: AuthUpdateSchema, session: AsyncSession = Depends(get_db)):
    auth_repo = AuthRepository(session)
    auth_service = AuthService(auth_repo)
    try:
